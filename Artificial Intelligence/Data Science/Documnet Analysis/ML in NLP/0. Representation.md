...
In contrast, a **sparse representation** might record only the non-zero entries and their positions:
...
2. **Computational efficiency**: Operations can be made faster because you only operate on non-zero values.
...
**Definition**:
Pointwise Mutual Information (PMI) is a measure of association used in information theory and statistics. It quantifies the degree to which the occurrence of two events X and Y deviates from what we would expect if they were independent. In the context of language and NLP, PMI often refers to the association between two words.
...
1. **Word Associations**: PMI is commonly used to find strong word associations in corpora. Words that frequently appear together will have a high PMI value.
