...
```python
function forwardChecking(csp, assignment):
    ...
    for value in orderDomainValues(var, assignment, csp):
        if isConsistent(var, value, assignment):
            ...
            inferences = {}
            for neighbor in getNeighbors(var, csp):
                if neighbor not in assignment:
                    for neighbor_value in csp.values[neighbor]:
                        if not isConsistent(neighbor, neighbor_value, assignment):
                            # remove value in neighbor that makes inconsistent (forward-check: constraint propagation technique)
                            csp.values[neighbor].remove(neighbor_value)
                    # if no values left for neighbor to choose, we enter a deadend and should backtrack
                    if len(csp.values[neighbor]) == 0:
                        return None
                    inferences[neighbor] = csp.values[neighbor]

            result = forwardChecking(csp, assignment)
            if result is not None:
                return result

            # Backtrack
            # undo the effects of the most recent assignment and any inferences that were made during forward checking
            assignment.pop(var)
            for neighbor, values in inferences.items():
                csp.values[neighbor].update(values)
