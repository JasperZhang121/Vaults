...
> 6. Using $P(C_i|X) \propto P(X|C_i)P(C_i)$
> 	Calculate $P(X = (middle-aged, fair) | buys = yes) * P(buys = yes)$

P(X = (middle_aged, fair)) = P(X = (middle_aged, fair) | buys_computer = yes) * P(buys_computer = yes) = (1/3 * 3/7) = 1/7

> 7. Calculate $P(X = (middle-aged, fair) | buys = no) * P(buys = no)$

P(X = (middle-aged, fair) | buys = no) * P(buys = no) = 1/4 * 4/7 = 1/7

> Calculate the likelihood probability P(age=middle_aged| buys=no)
