...
Several bootstrap methods: a common one is .632 bootstrap. A data set with $d$ tuples is sampled $d$ times, with replacement, resulting in a training set of $d$ samples. The data tuples that did not make it into the training set end up forming the test set. About <mark style="background: #BBFABBA6;">63.2% of the original data end up in the training set, and the remaining 36.8% form the test set</mark> (since $(1 - 1/d)^d \approx e^{-1} = 0.368$ if $d$ is very large).
