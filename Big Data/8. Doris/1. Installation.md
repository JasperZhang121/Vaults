...
**Notes:**
1. FE's disk is primarily used for <mark style="background: #FFB8EBA6;">storing metadata, including logs and images</mark>, typically ranging from a few hundred MB to several GB.
2. BE's disk is mainly used for <mark style="background: #FFF3A3A6;">storing user data</mark>. The total disk space should be calculated as the total user data * 3 (for three replicas), with an additional <mark style="background: #ABF7F7A6;">40% reserved for background compaction and storing intermediate data</mark>.
3. Multiple BE instances can be deployed on one machine, but <mark style="background: #BBFABBA6;">only one FE instance can be deployed per machine</mark>. For three replicas, at least three machines should each host one BE instance.
...
You can also monitor the system by accessing port <mark style="background: #D2B3FFA6;">8030</mark>. The account is `root` and the password is empty by default.
