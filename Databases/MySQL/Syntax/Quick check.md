...
| **Keyword**         | **Description**                                                          |
| ------------------- | ------------------------------------------------------------------------ |
| `INT`               | Integer data type used for primary keys or numeric fields.               |
| `AUTO_INCREMENT`    | Automatically increments the value for each new row.                     |
| `VARCHAR`           | Variable-length string with a specified maximum length.                  |
| `NOT NULL`          | Ensures the field must contain a value (cannot be `NULL`).               |
| `UNIQUE`            | Ensures all values in the column are unique across rows.                 |
| `DATE`              | Stores date values in `YYYY-MM-DD` format.                               |
| `DECIMAL`           | Stores precise decimal numbers with defined precision (e.g., salary).    |
| `ENUM`              | Defines a list of allowable values for a field.                          |
| `TINYINT(1)`        | Stores small integers, often used for boolean or status flags.           |
...
| `ON UPDATE`         | Automatically updates the timestamp whenever the row is updated.         |
| `CHECK`             | A constraint to validate data against a condition (e.g., `salary >= 0`). |
| `PRIMARY KEY`       | A unique identifier for each row in the table.                           |
| `FOREIGN KEY`       | Establishes a relationship between two tables.                           |
| `REFERENCES`        | Used with `FOREIGN KEY` to point to a primary key in another table.      |
| `INDEX`             | Speeds up search queries by indexing columns.                            |
| `FULLTEXT INDEX`    | Index for faster full-text searches on large text fields.                |
| `CONSTRAINT`        | Adds conditions for data validation (e.g., valid email format).          |
| `ENGINE=InnoDB`     | Specifies the storage engine for transaction-safe tables.                |
| `CHARSET=utf8mb4`   | Sets the character set to support multi-byte characters like emojis.     |
