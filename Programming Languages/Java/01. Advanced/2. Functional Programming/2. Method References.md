...
So, what is a **method reference**? If <mark style="background: #FFB8EBA6;">a method's signature matches that of the interface method, we can pass the method reference directly</mark>.
...
This is because instance methods have an implicit `this` parameter. When the `String.compareTo()` method is actually called, the first <mark style="background: #FFB8EBA6;">implicit argument</mark> is always `this`, which is equivalent to a static method like this:
