...
HTTP protocol is a request-response protocol based on TCP. It's very simple. First, use the Chrome browser to view the Sina homepage, then select View - Developer - Inspect Elements to see the HTML:
```
html
```
Switch to Network, reload the page, and you can see every request and response made by the browser:
```
http
```
Using the Chrome browser, it's convenient to debug web applications.
...
The HTTP request sent by the browser is as follows:
```
GET / HTTP/1.1
...
Usually, the first resource obtained by the browser is an HTML web page. In the web page, if JavaScript, CSS, images, videos, or other resources are embedded, the browser will request the corresponding resources from the server based on the resource's URL.

For detailed information about the HTTP protocol, please refer to the book *HTTP: The Definitive

 Guide* or the Mozilla developer website.
