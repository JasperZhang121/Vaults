...
In this section, we will not discuss server-side HTTP programming, because server-side HTTP programming essentially involves writing a web server, which is a very complex system and is the core content of JavaEE development. We will study this in more detail in later chapters.

In this section, we only discuss client-side HTTP programming.
